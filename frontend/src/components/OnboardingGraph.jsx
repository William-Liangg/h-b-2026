  ...
  return (
    ...
    >
      ...
      <div className={`${isMainNode ? 'p-4' : 'p-3'}`}>
        ...
        {isAuxiliary && (
          <div className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-purple-500 flex items-center justify-center text-[10px] text-white shadow-lg" title="Atlas Insight">
            ✨
          </div>
        )}
        {isBacklink && (
          <div className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-amber-500 flex items-center justify-center text-[10px] text-white shadow-lg" title="Linked File">
            🔗
          </div>
        )}
        ...
        {isMainNode && data.summary && (
          ...
          >
            {data.summary}
            {!textExpanded && data.summary.length > 80 && (
              <span className="text-cyan-400 ml-1 text-[10px]">▼</span>
            )}
            {textExpanded && (
              <span className="text-cyan-400 ml-1 text-[10px]">▲</span>
            )}
        ...
        {isSubNode && data.type && (
          <div className={`text-[10px] ${getTypeBadgeColor()} uppercase tracking-wide mt-1 flex items-center gap-1`}>
...
function inferExportType(name) {
  ...
  if (/^[A-Z][a-zA-Z0-9]*(?:Component|Provider|Context|Service|Manager|Handler|Controller|Factory|Builder|Store|Model|View|Page|Screen|Modal|Dialog|Form|Button|Input|Card|List|Item|Panel|Layout|Wrapper|Container|Router|Middleware|Plugin|Adapter|Client|Server|Worker|Queue|Cache|Logger|Config|Utils?|Helper|Validator|Parser|Formatter|Converter|Mapper|Reducer|Selector|Action|Slice|Saga|Thunk|Effect|Observer|Listener|Subscriber|Publisher|Emitter|Stream|Channel|Socket|Connection|Session|Auth|User|Admin|Api|Db|Sql|Http|Rest|Grpc|Graphql)$/.test(name)) {
    return { type: 'Class', icon: '🏛️' }
  }
  // Generic PascalCase (likely class/component)
  if (/^[A-Z][a-z]+(?:[A-Z][a-z]+)+$/.test(name) || /^[A-Z][a-zA-Z0-9]+$/.test(name)) {
    return { type: 'Class', icon: '🏛️' }
  }
  // Async functions (xxxAsync or fetchXxx, loadXxx, getXxx, postXxx, etc.)
  if (/(?:Async|Promise)$/.test(name) || /^(?:fetch|load|get|post|put|patch|delete|send|receive|request)[A-Z]/.test(name)) {
  ...
  if (/^(?:format|parse|convert|transform|to|from|map|reduce|filter)[A-Z]/.test(name)) {
...
function generateLayout(steps, expandedNodes, auxiliaryNodes = {}) {
  ...
  const nodeSpaces = steps.map((step) => {
    ...
    if (subNodes.length === 0) {
      ...
      dependencies.slice(0, 3).forEach(dep => {
        if (!subNodes.find(s => s.label === dep)) {
          subNodes.push({ label: dep, type: 'Dependency', icon: '🔗', isBacklink: true })
        }
      })
    ...
    const subNodeSpace = isExpanded ? Math.max(0, (subNodes.length - 1) * SUB_NODE_HEIGHT) : 0
    const auxNodeSpace = auxNodes.length > 0 ? Math.max(0, (auxNodes.length - 1) * AUX_NODE_HEIGHT) : 0
  ...
  nodeSpaces.forEach(({ step, subNodes, auxNodes, isExpanded, totalSpace }, index) => {
    const mainNodeId = step.file
    const y = currentY
...
export default function OnboardingGraph({ steps, onNodeClick, onNodeSelect, highlightedFiles, auxiliaryNodes }) {
  ...
  useMemo(() => {
    const { nodes: newNodes, edges: newEdges } = generateLayout(steps || [], expandedNodes, auxiliaryNodes)
    setNodes(newNodes)
    setEdges(newEdges)
    ...
    <div className="h-full w-full relative" style={{ background: '#09090b' }}>
      ...
      <div className="absolute top-4 left-4 bg-zinc-950/80 backdrop-blur-sm border border-zinc-800 rounded-lg px-3 py-2 text-[11px] text-zinc-500 pointer-events-none" style={{ zIndex: 10 }}>
        ...
        <div className="flex flex-wrap items-center gap-3 mt-1.5 pt-1.5 border-t border-zinc-800">
          <span className="text-zinc-600">Drag to select • Shift+click multi-select • Middle-click to pan</span>
        </div>
      </div>
    </div>
  )
